...
(function (H) {
  H.wrap(H.Chart.prototype, 'init', function (proceed, userOptions, callback) {
    ...
    var n = data.length;
    var binCountSturges = (Math.ceil( Math.log(n) / Math.LN2 + 1 )) * 2; // optimal # bins, Sturges' formula
    ...
    var binWidth = normalizeTickInterval(binWidthSturges,
    		null, getMagnitude(binWidthSturges)); // normalized bin width
    ...
    var options = H.merge(
      { // default settings
        ...
        plotOptions: { column: { pointPadding: 0, groupPadding: 0, borderWidth: 1,
        		pointInterval: binWidth, pointStart: binLow+binWidth/2, stacking: 'normal' } },
        tooltip: { formatter: function() {
          var upperBoundSymbol = (this.point.histBin === this.series.data.length-1) ? '≤' : '<';
          return this.point.histFrom+'≤x'+
          		upperBoundSymbol+this.point.histTo+'<br/>n=<b>'+this.y+'</b>';
        }}
      },
